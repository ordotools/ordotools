...
class Feast:
    """
    New Data Model using strictly typed slots for memory efficiency.
    """
    id: str
    date: datetime
    rank_numeric: int
    rank_verbose: str
    color: str
    office_type: Union[str, bool] = False
    
    # Nested properties
    mass_properties: Dict = field(default_factory=dict)
    alt_mass_properties: Dict = field(default_factory=dict)
    nobility: List[int] = field(default_factory=list)
    
    # Commemorations are now a list of dictionaries
    commemorations: List[Dict] = field(default_factory=list)
    
    # Divine Office Parts
    hours: Dict = field(default_factory=dict)
    matins: Dict = field(default_factory=dict)
    lauds: Dict = field(default_factory=dict)
    prime: Dict = field(default_factory=dict)
    little_hours: Dict = field(default_factory=dict)
    vespers: Dict = field(default_factory=dict)
    compline: Dict = field(default_factory=dict)

    # Logic-specific fields (needed for expand_octaves in main.py)
    day_in_octave: int = 0
    infra_octave_name: str = ""
    fasting: bool = False
    abstinence: bool = False

    name: str = ""

    @property
    def is_octave(self) -> bool:
        """Determines octave status on the fly."""
        return "Oct" in self.rank_verbose

    def get_mass(self, name: str = "Ad Missam") -> Dict:
        """Handles both single mass and multiple mass structures."""
        if name in self.mass_properties:
            return self.mass_properties[name]
        return self.mass_properties


# NOTE: this is the old stuff from the refactor
# from datetime import datetime
#
#
# class Feast:
